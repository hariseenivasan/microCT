...
//fwrite(pps, 1, sizeof(pps), f);
  ...
  picture->data[0] = (uint8_t*)pixels+(c->width)*3*((c->height)-1);
   sws_scale(img_convert_ctx, picture->data, 
		...
		if(out_size<0)
		{
		fprintf(stderr, "Problem encoding"); goto thread_exit;
		}
...
#endif
{
	PMYDATA threadData = (PMYDATA) lpParam;
...
void drawScene(void)
{
	...
	/*
	   ...
	   glColor3f(1,1,1);
	   char str[64];
	   sprintf(str, "Red - X  GREEN -Y  PINK - Z");
	   drawString(str, 0.0, 0.0, font);
	   glPopMatrix();
	 */
...
int main( int argc, char** argv )
{
				  ...
				  if(hThread == NULL)
 {
    ...
    if (s != 0)
	 ...
	 s=
pthread_create (&thread_id, &pta,
                MyThreadFunction, &myThreadData);
	 suspend
#endif				
